...
function summaryHtml({ yesterday, month, day, ops, counts, alertsItems }) {
  const tr = (k, v) => `<tr><td style="padding:4px 10px;color:#555">${k}</td><td style="padding:4px 10px"><b>${v}</b></td></tr>`;
  const tbl = (rows) => `<table style="border-collapse:collapse;width:100%;max-width:560px;border:1px solid #eee">${rows}</table>`;
  const dayBlock = day
    ? tbl(tr('Status', day.status) + tr('Loading (parts)', `${num(day.load_parts)} · ${num(day.load_trucks)} trucks`)
        + tr('Unloading (ton)', `${num(day.unload_ton)} · ${num(day.unload_trucks)} trucks`)
        + tr('QC / PDI (parts)', num(day.qc_parts)) + tr('Manpower', num(day.mp_actual)) + tr('Transport trips', num(day.trips)))
    : `<p style="color:#999">No entry recorded for ${yesterday}.</p>`;
  const mtdBlock = tbl(tr('Operating days', ops.opDays) + tr('Loading (parts)', num(ops.load_parts))
    + tr('Unloading (ton)', num(ops.unload_ton)) + tr('QC / PDI (parts)', num(ops.qc_parts))
    + tr('MG achievement', `${ops.mgAchievement}% (${ops.mg_met_days}/${ops.qcDays} QC days)`)
    + tr('Manpower utilization', `${ops.utilization}%`));
  const pending = `<p style="line-height:1.7">Pending approvals: <b>${counts.pending_approvals}</b> &nbsp;·&nbsp;
    Open discrepancies: <b>${counts.open_discrepancies}</b> &nbsp;·&nbsp; Pending MP requests: <b>${counts.pending_mp_requests}</b>
    &nbsp;·&nbsp; Pending leaves: <b>${counts.pending_leaves}</b> &nbsp;·&nbsp; Docs expiring (45d): <b>${counts.expiring_docs}</b>
    &nbsp;·&nbsp; Open CAPA: <b>${counts.open_capa}</b></p>`;
  const alertsBlock = alertsItems.length
    ? `<h3 style="margin:16px 0 4px">Alerts</h3><ul>${alertsItems.map(i => `<li><b style="color:${i.level === 'critical' ? '#b00020' : '#a06a00'}">${i.title}</b> — ${i.detail || ''}</li>`).join('')}</ul>`
    : '';
  return `<div style="font-family:Arial,Helvetica,sans-serif;color:#222">
    <h2 style="margin:0 0 2px">Drona ValueChain — Daily Operations Summary</h2>
    <div style="color:#666;margin-bottom:14px">${yesterday} · month ${month}</div>
    <h3 style="margin:8px 0 4px">Yesterday (${yesterday})</h3>${dayBlock}
    <h3 style="margin:16px 0 4px">Month-to-date (${month})</h3>${mtdBlock}
    <h3 style="margin:16px 0 4px">Pending</h3>${pending}${alertsBlock}
    <p style="color:#999;font-size:12px;margin-top:16px">Automated daily summary from the JMC Operations Tracker.</p></div>`;
}
...
module.exports = { evaluate, runDigest, runDailySummary, notifyRealtime, startScheduler };
